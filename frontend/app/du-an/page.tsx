    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
          ...
          >
            {stats.map((stat, index) => (
              <motion.div key={index} className="text-center" variants={itemVariants}>
                <div className="w-20 h-20 bg-blue-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white dark:border-gray-900 shadow-md">
      ...
      <section className="py-24 bg-gray-50 dark:bg-gray-950">
        <div className="container">
          <motion.div 
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-6 text-gray-900 dark:text-gray-50 font-serif">Các Dự Án Nổi Bật</h2>
          ...
          >
            {projects.map((project) => (
              <motion.div key={project.id} variants={itemVariants}>
                <Card className="h-full flex flex-col group overflow-hidden border-2 border-transparent hover:border-blue-500 hover:shadow-2xl transition-all duration-300 rounded-2xl dark:bg-gray-900 dark:border-gray-800">
                  <CardHeader className="p-0">
                    <div className="relative aspect-[16/10] overflow-hidden">
                      ...
                      />
                       ...
                       <Badge variant="secondary" className="absolute top-4 left-4 text-sm bg-white/90 text-black dark:bg-gray-800 dark:text-white">{project.category}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 flex flex-col flex-grow">
                    <CardTitle className="text-2xl font-bold text-gray-900 dark:text-gray-50 mb-3 leading-tight">{project.title}</CardTitle>
                    <p className="text-gray-600 dark:text-gray-400 mb-6 flex-grow">{project.description}</p>
                    
                    <div className="mt-auto">
                        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Chuyên gia phụ trách:</p>
                        <div className="flex items-center space-x-2">
                            {project.mentors.map(mentor => (
                                <div key={mentor.name} className="flex items-center space-x-2">
                                    <Image src={mentor.avatar} alt={mentor.name} width={32} height={32} className="rounded-full border-2 border-white dark:border-gray-700"/>
                                    <span className="text-sm text-gray-800 dark:text-gray-200">{mentor.name}</span>
                                </div>
                            ))}
                        </div>
                    </div>
      ...
      <section className="py-24 bg-white dark:bg-gray-900">
        <div className="container text-center">
           ...
           viewport={{ once: true }}
           >
             <h2 className="text-4xl md:text-5xl font-bold mb-6 text-gray-900 dark:text-gray-50 font-serif">Bạn có dự án cần triển khai?</h2>
